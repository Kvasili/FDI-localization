...
class StationaryWindowSHAP():
    ...
    def __init__(self, model, window_len, B_ts, test_ts, B_mask=None, B_dem=None,
        ...
        self.all_ts = np.concatenate(
            (self.background_ts, self.test_ts), axis=0)
        ...
        self.background_data = self.data_prepare(
            ts_x=self.background_ts, dem_x=self.background_dem, start_idx=0)
        self.test_data = self.data_prepare(
            ts_x=self.test_ts, dem_x=self.test_dem, start_idx=self.num_background)
    ...
    def data_prepare(self, ts_x, dem_x=None, start_idx=0):
        ...
        x_ = [
            [i]*total_num_features for i in range(start_idx, start_idx + ts_x.shape[0])]
    ...
    def wraper_predict(self, x):
        ...
        dem_x, ts_x = x[:, :self.num_dem_ftr].copy(
        ), x[:, self.num_dem_ftr:].copy()

        # initializing the value of all arrays
        ts_x_ = np.zeros(
            (x.shape[0], self.all_ts.shape[1], self.all_ts.shape[2]))
        mask_x_ = np.zeros_like(ts_x_)
        dem_x_ = np.zeros_like(dem_x, dtype=float)
        tstep = np.ones((x.shape[0], self.all_ts.shape[1], 1)) * \
            np.reshape(
                np.arange(0, self.all_ts.shape[1]), (1, self.all_ts.shape[1], 1))
        ...
        for i in range(x.shape[0]):
            ...
            for t in range(self.num_ts_step):
                for j in range(self.num_ts_ftr):
                    ...
                    mask_x_[
                        i, t, j] = None if self.all_mask is None else self.all_mask[ind, t, j]
    ...
    def shap_values(self, num_output=1):
        self.explainer = shap.KernelExplainer(
            self.wraper_predict, self.background_data)
        ...
        self.ts_phi = np.repeat(self.ts_phi/self.window_len,
                                self.window_len, axis=2)[:, :, :self.num_ts_step, :]
...
class SlidingWindowSHAP():
    ...
    def wraper_predict(self, x, start_ind=0):
        ...
        tstep = np.ones((x.shape[0], self.num_ts_step, 1)) * \
            np.reshape(np.arange(0, self.num_ts_step),
                       (1, self.num_ts_step, 1))
    ...
    def shap_values(self, num_output=1, nsamples='auto'):
        ...
        num_sw = np.ceil((seq_len - self.window_len) /
                         self.stride).astype('int') + 1
        ts_phi = np.zeros(
            (self.num_test, num_sw, 2, self.background_ts.shape[2]))
        ...
        for stride_cnt in range(num_sw):
            ...
            def predict(x): return self.wraper_predict(
                x, start_ind=stride_cnt * self.stride)

            # Running SHAP
            self.explainer = shap.KernelExplainer(
                predict, self.background_data)
            shap_values = self.explainer.shap_values(
                self.test_data, nsamples=nsamples)
            shap_values = np.array(shap_values)

            # Extracting the SHAP values and storing them
            dem_shap_values_ = shap_values[:, :, :self.num_dem_ftr]
            ts_shap_values = shap_values[:, :, self.num_dem_ftr:]
            ts_shap_values = ts_shap_values.reshape(
                (num_output, self.num_test, 2, self.num_ts_ftr))
        ...
        ts_phi_agg = np.empty(
            (self.num_test, num_sw, self.num_ts_step, self.num_ts_ftr))
        ts_phi_agg[:] = np.nan
        for k in range(num_sw):
            ts_phi_agg[:, k, k * self.stride:k * self.stride +
                       self.window_len, :] = ts_phi[:, k, 0, :][:, np.newaxis, :]
...
class DynamicWindowSHAP():
    ...
    def __init__(self, model, delta, n_w, B_ts, test_ts, B_mask=None, B_dem=None,
        ...
        # For binary time window algorithm, samples should be fed to the algorithm one-by-one
        assert self.num_test == 1
        self.delta = delta
        self.n_w = n_w
        self.split_points = [[self.num_ts_step - 1]] * \
            self.num_ts_ftr  # Splitting points
    ...
    def data_prepare(self, ts_x, dem_x=None, start_idx=0):
        assert len(ts_x.shape) == 3
        assert dem_x is None or len(dem_x.shape) == 2
        total_num_features = self.num_dem_ftr + \
            sum(self.num_window)  # Specific to Binary Time Window
    ...
    def wraper_predict(self, x):
        ...
        # Specific to Binary Time Window
        temp_ts_x = np.zeros(
            (ts_x.shape[0], max(self.num_window), self.num_ts_ftr), dtype=int)
        for i in range(self.num_ts_ftr):
            temp_ts_x[:, :self.num_window[i], i] = ts_x[:, sum(
                self.num_window[:i]):sum(self.num_window[:i+1])]
        ts_x = temp_ts_x
    ...
    def shap_values(self, num_output=1, nsamples_in_loop='auto', nsamples_final='auto'):
        ...
        while flag:
            ...
            self.num_window = [len(self.split_points[i])
                               for i in range(self.num_ts_ftr)]
            ...
            shap_values = self.explainer.shap_values(
                self.test_data, nsamples=nsamples)
            shap_values = np.array(shap_values)
            # Extracting dem SHAP values
            dem_phi = shap_values[0, :, :self.num_dem_ftr]
            # Extracting ts SHAP values
            ts_shap_values = shap_values[:, :, self.num_dem_ftr:]

            # Checking the maximum number of windows condition
            if max(self.num_window) >= self.n_w:
                break
        ...
        self.explainer = shap.KernelExplainer(
            self.wraper_predict, self.background_data)
        shap_values = self.explainer.shap_values(
            self.test_data, nsamples=nsamples_final)
        shap_values = np.array(shap_values)
        # Extracting dem SHAP values
        dem_phi = shap_values[0, :, :self.num_dem_ftr]
        # Extracting ts SHAP values
        ...
        for i in range(self.num_ts_ftr):
            for j in range(self.num_window[i]):
                ...
                end_ind = self.split_points[i][j] + \
                    int((j + 1) / self.num_window[i])
                ts_phi[0, start_ind:end_ind, i] = ts_shap_values[0, :,
                                                                 sum(self.num_window[:i]) + j] / (end_ind - start_ind)
        self.dem_phi = dem_phi
        self.ts_phi = ts_phi
